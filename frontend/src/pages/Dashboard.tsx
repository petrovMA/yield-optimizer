...
const VAULT_ABI = [
    ...
    'function isCompoundPool(address) external view returns (bool)',
];

const AAVE_POOL_ABI = [
    'function userBalances(address asset, address user) external view returns (uint256)',
];

const COMPOUND_POOL_ABI = [
...
export default function Dashboard() {
    ...
    const [contractData, setContractData] = useState<ContractData>({
        ...
        allPools: [],
        totalValueLocked: 10000000n, // Mock 10,000 USDT (6 decimals)
        isLoading: true,
        error: null,
    });
        ...
        const fetchData = async () => {
            ...
            for (let i = 0; i < SEPOLIA_RPCS.length; i++) {
                try {
                    ...
                    // Fetch real TVL from active pool
                    let tvl = 0n;
                    try {
                        if (activePool !== ethers.ZeroAddress) {
                            const isCompound = await vaultContract.isCompoundPool(activePool);

                            if (isCompound) {
                                // Compound pool: userBalances(address user)
                                const poolContract = new ethers.Contract(activePool, COMPOUND_POOL_ABI, provider);
                                tvl = await poolContract.userBalances(AUTO_YIELD_VAULT);
                            } else {
                                // Aave pool: userBalances(address asset, address user)
                                const assetAddress = await vaultContract.asset();
                                const poolContract = new ethers.Contract(activePool, AAVE_POOL_ABI, provider);
                                tvl = await poolContract.userBalances(assetAddress, AUTO_YIELD_VAULT);
                            }
                        }
                    } catch (err) {
                        console.warn('Failed to fetch TVL, using 0:', err);
                        tvl = 0n;
                    }
            ...
            <header className="relative z-10 border-b border-[#00f3ff]/20 bg-black/60 backdrop-blur-xl">
                <div className="container mx-auto px-6 py-4">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-4">
                            <div
                                className="flex items-center gap-2 px-4 py-2 bg-[#0aff00]/10 border border-[#0aff00]/30 rounded-lg">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <Card className="bg-black/40 border-[#627eea]/30 backdrop-blur-xl p-6">
                        ...
                        <div className="mb-6">
                            ...
                            <a
                                href={getExplorerUrl(AUTO_YIELD_VAULT)}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="text-xs font-mono text-gray-500 hover:text-[#00f3ff] underline decoration-dotted transition-colors break-all block"
                            >
                                {AUTO_YIELD_VAULT}
                            </a>
                        </div>
                        ...
                        {contractData.isLoading ? (
                            ...
                            <div className="space-y-6">
                                {/* TVL */}
                                <div>
                                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-2">Total Value
                                        Locked</p>
                                    <div className="flex items-baseline gap-2">
                    <span className="text-4xl font-bold font-mono text-white">
                      ${Number(ethers.formatUnits(contractData.totalValueLocked, 18)).toLocaleString('en-US', {
                    ...
                    <span className="text-5xl font-bold font-mono text-[#0aff00]">
                      {rayToPercent(contractData.currentRate)}%
                                        ...
                                        <div className="flex items-center gap-3 mb-2">
                                            ...
                                            <div>
                                                <p className="font-bold text-lg"
                                                   style={{color: activePoolConfig.color}}>
                                ...
                                <div className="border-t border-gray-800 pt-4 space-y-3">
                                    <Button
                                        className="w-full bg-gradient-to-r from-[#00f3ff] to-[#0aff00] text-black font-bold hover:shadow-[0_0_20px_rgba(0,243,255,0.5)]">
                                        <ArrowDownRight className="w-4 h-4 mr-2"/>
                                        Deposit USDT
                                    </Button>
                                    <Button
                                        className="w-full bg-black/60 border-2 border-[#00f3ff] text-[#00f3ff] font-bold hover:bg-[#00f3ff]/10">
                                            ...
                                            <div className="mb-3">
                                                <div className="flex items-center justify-between gap-2 mb-1">
                                                    ...
                                                    <div className="flex gap-1 flex-shrink-0">
                                                        {isActive && (
                                                            <Badge
                                                                className="bg-[#627eea]/20 text-[#627eea] border-[#627eea]/50 text-xs">
                                                                ACTIVE
                                                            </Badge>
                                                        )}
                                                        {isBest && contractData.shouldRebalance && (
                                                            <Badge
                                                                className="bg-[#0aff00]/20 text-[#0aff00] border-[#0aff00]/50 text-xs animate-pulse">
                                                                TARGET
                                                            </Badge>
                                                        )}
                                                    </div>
                        ...
                        <span className="text-3xl font-bold font-mono text-[#0aff00]">
                          {rayToPercent(pool.rate)}%
                                ...
                                <div className="border-t border-gray-800 pt-4 mt-6">
                                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-2">Rebalance
                                        Threshold</p>
                    ...
                    <Card
                        className={`bg-black/40 backdrop-blur-xl p-6 ${isPaused ? 'border-gray-500/30' : 'border-[#00f3ff]/30'}`}>
                        <TooltipProvider>
                            <Tooltip>
                                <TooltipTrigger asChild>
                                    <div className={isPaused ? 'cursor-help' : ''}>
                                        <div className="flex items-center gap-2 mb-6">
                                            <Activity
                                                className={`w-5 h-5 ${isPaused ? 'text-gray-500' : 'text-[#00f3ff]'}`}/>
                                            <h2 className={`text-xl font-bold ${isPaused ? 'text-gray-500' : 'text-[#00f3ff]'}`}>REACTIVE
                                                SCHEDULER</h2>
                                        </div>
                                        ...
                                        <div className="space-y-6">
                                            ...
                                            <div>
                                                <p className="text-xs text-gray-500 uppercase tracking-wider mb-4 text-center">Next
                                                    Cron Check</p>
                                            ...
                                            {/* Decision Matrix */}
                                            <div className="border-t border-gray-800 pt-4">
                                                <p className="text-xs text-gray-500 uppercase tracking-wider mb-3">Decision
                                                    Matrix</p>
                                                <div className="space-y-3">
                                                    ...
                                                    <div className="bg-black/60 rounded-lg p-3 border border-gray-700">
                                                        <p className="text-xs text-gray-500 mb-1">Best Potential
                                                            Rate</p>
                                                    ...
                                                    <div
                                                        className="bg-black/60 rounded-lg p-3 border border-[#00f3ff]/50">
                                            ...
                                            <div className="border-t border-gray-800 pt-4">
                                                {contractData.shouldRebalance ? (
                                                    <div
                                                        className="bg-gradient-to-br from-[#0aff00]/20 to-black/60 border-2 border-[#0aff00] rounded-xl p-4 shadow-[0_0_30px_rgba(10,255,0,0.3)]">
                                                        <div className="flex items-center gap-2 mb-3">
                                                            <AlertTriangle
                                                                className="w-5 h-5 text-[#0aff00] animate-pulse"/>
                                                            <p className="font-bold text-[#0aff00] text-lg">REBALANCE
                                                                REQUIRED</p>
                                                        </div>
                                                    ...
                                                    <div
                                                        className="bg-gradient-to-br from-[#00f3ff]/20 to-black/60 border-2 border-[#00f3ff] rounded-xl p-4">
                                                        <div className="flex items-center gap-2 mb-2">
                                                            <CheckCircle2 className="w-5 h-5 text-[#00f3ff]"/>
                                                            <p className="font-bold text-[#00f3ff] text-lg">YIELD
                                                                OPTIMIZED</p>
                                                        </div>
